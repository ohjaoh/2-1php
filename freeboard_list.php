        ...
        <section>
            ...
            <div id="freeboard_box">
                ...
                <ul id="freeboard_list">
                    ...
                    /* DB에서 전체 게시글 가져오기
                      • freeboard 테이블에서 레코드 일련번호인 num을 기준으로 내림차순한 전체 레코드를 가져와 $result에 저장.
                    ...
                    for ($i = $start; $i < $start + $scale && $i < $total_record; $i++) { // DB에서 글목록을 가져오기 위한 루프설정.
                        ...
                        <li>
                            ...
                            <span class="col2">
                                <a href="freeboard_view.php?num=<?= $num ?>&page=<?= $page ?>"><?= $subject ?></a></span>
                ...
                <ul id="page_num"> 	
                    ...
                    if ($total_page >= 2 && $page >= 2) {
                        ...
                        $new_page = $page - 1;
                        echo "<li><a href='freeboard_list.php?page=$new_page'>◀ 이전</a> </li>";
                    } else
                        echo "<li>&nbsp;</li>";
                    ...
                    if ($total_page >= 2 && $page != $total_page) {
                        $new_page = $page + 1;
                        echo "<li> <a href='freeboard_list.php?page=$new_page'>다음 ▶</a> </li>";
                    } else
                        echo "<li>&nbsp;</li>";
                    ?>
                </ul> <!-- page -->	    	
                <ul class="buttons">
                    <li><button onclick="location.href = 'freeboard_list.php'">목록</button></li>
                    <!-- <목록>, <글쓰기> 버튼 삽입
                    <button> 태그로 <목록> 버튼 삽입. 버튼 클릭 시 글 목록 보기 페이지(freeboard_list.php)로 이동하도록 링크 설정.
                    <글쓰기> 버튼은 if문의 조건식에 있는 $userid가 값을 가질 때, 즉 로그인 상태에서만 글쓰기페이지(freeboard_form.php)로 이동하도록 설정. $userid에 값이 없으면 로그인 후 버튼을 클릭하라는 경고 창 띄움. -->
                    <li>
                        <?php
                        if ($userid) {
                            ?>
                            <button onclick="location.href = 'freeboard_form.php'">글쓰기</button>
                            <?php
                        } else {
                            ?>
