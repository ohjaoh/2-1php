        ...
        <section>
            ...
            <div id="gongiboard_box">
                ...
                <ul id="gongiboard_list">
                    ...
                    /* DB에서 전체 게시글 가져오기
                      • gongiboard 테이블에서 레코드 일련번호인 num을 기준으로 내림차순한 전체 레코드를 가져와 $result에 저장.
                      • mysqli_num_rows( ) 함수를 이용하여 전체 레코드 수(게시글 수)를 $total_record에 저장.
                     */
                    $sql = "select * from gongiboard  order by num desc";
                    ...
                    for ($i = $start; $i < $start + $scale && $i < $total_record; $i++) { // DB에서 글목록을 가져오기 위한 루프설정.
                        ...
                        <li>
                            ...
                            <!-- 글 목록의 각 항목을 화면에 출력. $number는 게시글 번호, $subject는 제목, $_name_gong 글쓴이, $file_image는 첨부 파일 아이콘을 삽입하는 문자열, $regist_day는 작성 일시, $hit는 조회 수  -->
                            <span class="col2">
                                <a href="gongiboard_view.php?num=<?= $num ?>&page=<?= $page ?>"><?= $subject ?></a></span>
                            <span class="col3"><?= $name_gong ?></span>
                ...
                <ul id="page_num"> 	
                    ...
                    if ($total_page >= 2 && $page >= 2) {
                        ...
                        $new_page = $page - 1;
                        echo "<li><a href='gongiboard_list.php?page=$new_page'>◀ 이전</a> </li>";
                    } else
                        echo "<li>&nbsp;</li>";
...
// 게시판 목록 하단에 페이지 링크 번호 출력
                    for ($i = 1; $i <= $total_page; $i++) {
                        if ($page == $i) {     // 현재 페이지 번호 링크 안함
                            ...
                            echo "<li><a href='gongiboard_list.php?page=$i'> $i </a><li>";
                        }
                    }
                    if ($total_page >= 2 && $page != $total_page) {
                        $new_page = $page + 1;
                        echo "<li> <a href='gongiboard_list.php?page=$new_page'>다음 ▶</a> </li>";
                    } else
                        echo "<li>&nbsp;</li>";
                    ?>
                </ul> <!-- page -->	    	
                <ul class="buttons">
                    <li><button onclick="location.href = 'gongiboard_list.php'">목록</button></li>
                    <!-- <목록>, <글쓰기> 버튼 삽입
                    <button> 태그로 <목록> 버튼 삽입. 버튼 클릭 시 글 목록 보기 페이지(gongiboard_list.php)로 이동하도록 링크 설정.
                    <글쓰기> 버튼은 if문의 조건식에 있는 $userid가 값을 가질 때, 즉 로그인 상태에서만 글쓰기페이지(gongiboard_form.php)로 이동하도록 설정. $userid에 값이 없으면 로그인 후 버튼을 클릭하라는 경고 창 띄움. -->
                    <li>
                        <?php
                        if ($userlevel == 1) {
                            ?>
                            <button onclick="location.href = 'gongiboard_form.php'"> 글쓰기</button>
                            <?php
                        } else {
                            ?>
                            <a href="javascript:alert('권한이 없습니다!')"><button>글쓰기</button></a>
                            <?php
                        }
                        ?>
                    </li>
